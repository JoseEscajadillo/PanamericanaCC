...
class Juego:
    ...
    def iniciarJuego(self):
        ...
        clock = pygame.time.Clock()
        dificultad_incremento = [10, 30, 50]
        ...
        for episodio in range(self.episodios):
            ...
            while running :
                ...
                self.carril_state = np.array([])

                if len(self.enemigos_rects) == 1:
                        if self.enemigos_rects[0][0] == 145:
                            np.concatenate([self.carril_state, [0]])
                        elif self.enemigos_rects[0][0] == 345:
                            np.concatenate([self.carril_state, [1]])
                        elif self.enemigos_rects[0][0] == 545:
                            np.concatenate([self.carril_state, [2]])
                ...
                for i, enemigo in enumerate(self.enemigos):
                    ...
                    self.comp = [self.jugador.riel_player, self.state,direccion_xaut]
                    ...
                    if self.puntaje >= 50 and self.modo!=0:
                                qguard = self.qtable.reshape(-1, self.qtable.shape[-1])
                                df = pd.DataFrame(qguard)
                                df.to_excel(r"C:\Users\José\Proyecto POO\qtable.xlsx")  
                                self.gameOver()
    ...
    def pantalla_inicio(self):
        ...
        while True:
            ...
            self.dibujar_boton("CLOSE", self.ROJO_OSCURO, self.ROJO_CLARO, boton_cerrar)
            pygame.display.flip()
...
jugador = CarroJugador(carril= random.randint(0, 2), y=640, velocidad=5.0, ancho=120, alto=150)  
